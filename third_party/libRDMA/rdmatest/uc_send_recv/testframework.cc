...
void *run_server(void *arg) {
	...
	RdmaCtrl *rdma = bootstrap_uc_rdma(configs.node_id, config.network, config.port,
		thread_id, NULL, config.userBufSize, config.ucPerThread);
	memset((void *) rdma->conn_buf_, 0, config.userBufSize);
	...
	while(1) {
		...
		for(int nid = 0; nid < num_nodes; nid++){
			...
			int qid = _QP_ENCODE_ID(nid,uc_qp_i+thread_id*config.ucPerThread+UC_ID_BASE);
			// rdma->post_uc_doorbell(qid, config.batchSize, reqs);
			// rolling_iter+=config.batchSize;
			rdma->post_uc(qid,reqs);
			rdma->poll_cq(qid);
			rolling_iter++;
		}
		uc_qp_i++;
		if(uc_qp_i == config.ucPerThread) {
			uc_qp_i = 0;
		}
	}
	return NULL;
...
void *run_client(void *arg) {
	...
	int qid = _QP_ENCODE_ID(0, thread_id*config.ucPerThread+UC_ID_BASE);
	Qp *qp = rdma->qps_[qid];
	...
	while(1) {
		...
		// qp->poll_recv_cq();
		// rdma->post_conn_recv(qp->qp,
			// (void *) rdma->conn_buf_, config.userBufSize, rdma->rdma_device_->conn_buf_mr->lkey);
		// rolling_iter++;
		rolling_iter += rdma->poll_conn_recv_cqs(qid);
	}
...
int main(int argc, char *argv[]) {
	...
	config.readUcPerThread();
	config.readRecvUcPerThread();
