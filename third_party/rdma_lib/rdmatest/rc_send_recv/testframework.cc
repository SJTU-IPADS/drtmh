...
void *run_server(void *arg) {
	...
	while(1) {
		...
		for(int nid = 0; nid < num_nodes; nid++){
			...
			int qid = _QP_ENCODE_ID(nid,rc_qp_i+thread_id*config.rcPerThread+RC_ID_BASE);
			// rdma->post_rc_doorbell(qid, config.batchSize, reqs);
			// rolling_iter+=config.batchSize;
			rdma->post_rc(qid,reqs);
			rdma->poll_cq(qid);
			rolling_iter++;
		}
		rc_qp_i++;
		if(rc_qp_i == config.rcPerThread) {
			rc_qp_i = 0;
		}
	}
	return NULL;
...
void *run_client(void *arg) {
	...
	int qid = _QP_ENCODE_ID(0, thread_id*config.rcPerThread+RC_ID_BASE);
	Qp *qp = rdma->qps_[qid];
	...
	while(1) {
		if(rolling_iter >= config.iterationNum) {
			...
			double seconds = (end.tv_sec - start.tv_sec) + 
				(double) (end.tv_nsec - start.tv_nsec) / 1000000000;
			printf("main: Client %d: %.2f Mops\n", thread_id, config.iterationNum / seconds);
			rolling_iter = 0;
