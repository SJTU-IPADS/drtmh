...
def kill_servers(mac_set,exe):
    ...
    kill_cmd2 = "pkill %s" % prev_exe
    subprocess.call(["ssh", "-n","-f", mac_set[0], kill_cmd1],stdout = FNULL,stderr = subprocess.STDOUT)

    time.sleep(1)

    for i in xrange(len(mac_set)):
        try:
            subprocess.call(["ssh", "-n","-f", mac_set[i], kill_cmd],stdout = FNULL,stderr = subprocess.STDOUT)
            subprocess.call(["ssh", "-n","-f", mac_set[i], kill_cmd2],stdout = FNULL,stderr = subprocess.STDOUT)
    ...
    subprocess.call(["ssh","-n","-f","val@r715",'cd dongzy-deply;ansible-playbook kill.yml -e "exe=%s"' % exe], \
