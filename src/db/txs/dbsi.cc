...
namespace nocc {
    ...
    namespace db {
              ...
              TXHandler(c_id) // farther class
        {
            ...
            rpc_->register_callback(std::bind(&DBSI::get_rpc_handler,this,_1,_2,_3,_4),RPC_READ);
            rpc_->register_callback(std::bind(&DBSI::lock_rpc_handler,this,_1,_2,_3,_4),RPC_LOCK);
            rpc_->register_callback(std::bind(&DBSI::release_rpc_handler,this,_1,_2,_3,_4),RPC_RELEASE);
            rpc_->register_callback(std::bind(&DBSI::commit_rpc_handler2,this,_1,_2,_3,_4),RPC_COMMIT);
            rpc_->register_callback(std::bind(&DBSI::acquire_ts_handler,this,_1,_2,_3,_4),RPC_TS_ACQUIRE);
#if TS_USE_MSG == 1
            rpc_->register_callback(std::bind(&TSManager::ts_update_handler,ts_manager,
                                                      _1,_2,_3,_4),RPC_COMMIT + 1);
#endif
                ...
                retry:
                    ...
                    uint64_t DBSI::get_commit_ts(yield_func_t &yield,uint64_t &encoded_commit_ts) {
                        ...
                        *((uint64_t *)req_buf) = commit_ts;
                        //rpc_handler_->send_reqs(RPC_TS_ACQUIRE,sizeof(uint64_t),(char *)(&commit_ts),&master_id,1,cor_id_);
                    ...
                    void DBSI::commit_ts(uint64_t ts,yield_func_t &yield) {
                        ...
                        rpc_->append_req(req_buf,RPC_COMMIT + 1,sizeof(TSManager::UpdateArg),
                                         cor_id_,RRpc::REQ,master_id,nthreads);
                        return;
#endif
