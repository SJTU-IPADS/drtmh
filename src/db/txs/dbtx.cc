     ...
     TXHandler(c_id)
{
  ...
  //rpc_->register_callback(std::bind(&DBTX::log_clean_rpc_handler,this,_1,_2,_3,_4),RPC_LOGGING_CLEAN,true);
...
void DBTX::insert(int tableid, uint64_t key, char *val, int len) {
  ...
  RWSet::RWSetItem item;
  item.addr = (uint64_t *) (new char[vlen + META_LENGTH]);
...
#endif
  remoteset->log_remote(new_logger_,yield);
