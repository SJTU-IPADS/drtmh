  ...
  namespace oltp {
    ...
    namespace tpcc {
      ...
      void TpccWarehouseLoader::load() {
        ...
        try {
          ...
          for (uint i = GetStartWarehouse(partition_); i <= GetEndWarehouse(partition_); i++) {
            ...
            if (Encode(k).size() !=8) std::cerr << Encode(k).size() << std::endl;
        ...
        if (verbose) {
          std::cerr << "[TPCC] finished loading warehouse" << std::endl;
          std::cerr << "[TPCC]   * average warehouse record length: "
      ...
      void TpccDistrictLoader::load() {
        ...
        if (verbose) {
          cerr << "[TPCC] finished loading district" << endl;
          cerr << "[TPCC]   * average district record length: "
      ...
      void TpccCustomerLoader::load() {
        ...
        if (verbose) {
          cerr << "[TPCC] finished loading customer" << endl;
          cerr << "[TPCC]   * average customer record length: "
