...
namespace nocc {
  namespace oltp {
    ...
    namespace tpcc {
...
#if NAIVE == 1
          txn_result_t r = static_cast<TpccWorker *>(w)->txn_new_order_naive(yield);
#elif NAIVE == 2 || NAIVE == 3
          txn_result_t r = static_cast<TpccWorker *>(w)->txn_new_order_naive1(yield);
#else
      ...
      public:
      TpccStockLoader(unsigned long seed, int partition, MemDB *store)
        : BenchLoader(seed),
          TpccMixin(store) {
