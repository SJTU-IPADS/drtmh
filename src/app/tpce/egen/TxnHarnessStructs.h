...
typedef struct TBrokerVolumeTxnInput1
{
    // Transaction level inputs
  uint64_t broker_list[max_broker_list_len];
  char            sector_name[cSC_NAME_len+1];
  int8_t broker_num;  
}   *PBrokerVolumeTxnInput1,
      TBrokerVolumeFrame1Input1,  // Single-Frame transaction
      *PBrokerVolumeFrame1Input1;  // Single-Frame transaction
...
typedef struct TTradeResultTxnInput
{
    double      trade_price;
    TTrade      trade_id;
TTradeResultTxnInput(double d,TTrade x) :trade_price(d),trade_id(x) {}
