...
extern std::map<std::string,uint64_t> SecurityToId;
...
class TpceMainRunner : public BenchRunner {
 public:
  ...
  virtual void populate_cache() {
    ...
    LOG(3) << "loading location cache.";

    RThreadLocalInit();
    char *temp = (char *)Rmalloc(256);

    for(auto it = SecurityToId.begin();it != SecurityToId.end();++it) {
      std::string symbl = it->first;
    ...
    Rfree(temp);
    LOG(3) << "location cache load done.";
#endif
  }
};
